...
| - | ○ | claude | claude-stats のセッション数にサブエージェントセッションが含まれ過大計上される — Task ツールで起動されるサブエージェントも SessionStart hook で記録され同じ PR に紐づくため | [ADR-049](adr/049-session-count-excludes-subagent-sessions.md) |
