...
- [ ] パススルーモード ON 時にローカル tmux のステータスバーが非表示になる
- [ ] パススルーモード OFF 時にローカル tmux のステータスバーが通常表示に復帰する
- [ ] F12 手動トグルで上記の表示切り替えが動作する
- [ ] `tms` / `ssh` の自動パススルーで上記の表示切り替えが動作する
- [ ] パススルーモード OFF 時のステータスバー表示が操作対象レイヤー（LOCAL）を示す
- [ ] SSH window 以外でパススルーモードを手動有効化する機能（F12）が削除されている
- [ ] SSH 接続時に自動でリモート側の tmux が操作対象になる（F12 で LOCAL に切り替え可）
