...
- [x] watcher が launchd で常駐し、プロセスを kill しても自動で再起動される（KeepAlive / StartInterval は launchd に保留されるため不採用。StartCalendarInterval 毎分 + flock ガードで、kill 後は次の分の tick（最大 60 秒 + slack 数秒）で復活することを実測）
...
### herdr sidebar の agent 行でセッションを識別できるようにする

**コンポーネント**: herdr

sidebar の agent 一覧から目的のセッションを特定できない。行は `state_icon/workspace/tab` →
`terminal_title_stripped` → `agent` の 3 行あるが、`sidebar_width = 26`（既定）だと
セッション要約が全角 13 文字ほどで切れて読めず、3 行目の `agent` は全ペインが `claude` で
情報量がゼロ。ペインが 30 近くあると workspace 名と tab 番号だけが手がかりになる。

`sidebar_width` を 34（`sidebar_max_width` 44）へ広げてセッション要約を読める幅にし、
`[ui.sidebar.agents.rows_by_agent]` で claude のときだけ 2 行目を
`terminal_title_stripped`（`dim = false`）に差し替える。既定の `rows` は 2 行に減らして
`agent` を残すので、codex など claude 以外のペインは従来どおり agent 名で見分けられる。
行数が 3 → 2 に減るぶん、幅を広げても一覧の縦密度は上がる。

出典は sre-hub PR #2885（同じ設定を collapsed 運用で入れた例）。こちらは
`sidebar_start_collapsed` を設定しておらず常時展開なので、幅 34 は常に画面を食う。

**受け入れ条件**:

- [ ] `configs/herdr/config.toml` の `[ui]` が `sidebar_width = 34` / `sidebar_max_width = 44`
- [ ] `[ui.sidebar.agents.rows_by_agent]` の `claude` が 2 行で、2 行目が `terminal_title_stripped`
- [ ] `herdr config check` が `config: ok`
- [ ] `herdr server reload-config` が `status: applied`
- [ ] 実機: sidebar の claude ペイン行にセッション要約が切れずに出て、目的のセッションを一覧から特定できる
- [ ] 実機: claude 以外（codex）のペイン行は 2 行目に agent 名が出る
- [ ] 実機: 常時展開のまま幅 34 が許容できるか判断する（きつければ 26〜30 へ戻すか `sidebar_start_collapsed = true` を検討）
