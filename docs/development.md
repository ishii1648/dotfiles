...
ADR の `## 結果` セクションは、`docs/issues.md` の受け入れ条件を起点に書く。

```markdown
## 結果

**受け入れ条件の達成状況**（[issues.md](../issues.md) 対応行より）:
...
- ADR のステータスを「採用済み」にするタイミング = 受け入れ条件をチェックしたタイミング
- `issues.md` の受け入れ条件が正（single source of truth）。ADR には達成状況のみ記載し、条件の定義は複製しない
