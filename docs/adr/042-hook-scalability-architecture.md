...
`configs/claude/settings.json` の `hooks` セクションは肥大化を続けており、現在は 7 イベント・15 エントリ前後を抱える。さらに今後も観測系 hook（agent-telemetry 等）や安全フィルタの拡張が見込まれる。

```
SessionStart    → claude-pane-state.sh idle + workflow-session-start.sh + agent-telemetry(session-start, todo-cleanup)
...
Stop → check-uncommitted-on-feature.sh + claude-pane-state.sh idle + workflow-session-log.sh + agent-telemetry(stop)
SessionEnd → claude-pane-state.sh end + agent-telemetry(session-end)
```

すべての hook 登録は dotfiles の `configs/claude/settings.json` をソースとし、`setup.sh` が `~/.claude/settings.json` の `hooks` キーを同期する（ADR-041）。agent-telemetry（旧 `hitl-metrics`）等の外部 CLI が settings.json を直接書き換える経路は存在しない（案E の前提）。agent-telemetry は `install` で hook を自動登録する機能を持たず、登録は dotfiles 経由・検証のみ `agent-telemetry doctor` が担当する設計（[agent-telemetry setup.md](https://github.com/ishii1648/agent-telemetry/blob/main/docs/setup.md)）。claudedog から agent-telemetry への移行に伴い、`~/.claude/claudedog/hooks/*` 系の per-event hook は撤去済み（agent-telemetry は transcript ベースの後処理 `backfill`/`sync-db` で代替）。
