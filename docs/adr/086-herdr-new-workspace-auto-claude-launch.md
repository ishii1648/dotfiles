...
残る未検証事項だったもの（**ADR-088 で決着**）: 「herdr が popup を閉じる際にプロセスグループごと終了させるのではないか」という懸念は、実際にそのとおりだったことが ADR-088 の実装中に判明した。ただし `launch_claude_retry` は冒頭で `trap '' HUP` しているため生き残っており、この経路は結果的に正しく守られていた。逆に、同じ場所から `trap` 無しでバックグラウンド起動していた ADR-088 の pull は 1 行もログを残さず即死していた。詳細と対処（`( trap '' HUP; exec ... ) &` の形）は [ADR-088](088-auto-pull-default-branch-on-open.md) を参照。
