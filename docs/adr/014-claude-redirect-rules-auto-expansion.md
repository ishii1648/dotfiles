...
- **ADR-091 で上書き** — 本 ADR で追加した `head` / `tail` / `awk` / `mkdir` / `cp` / `for` / `while` / `python -c` / プロジェクト外 `.py` のルール、および `&&` / `$()` の全面禁止は削除した。prompt を減らさない、あるいは助言として誤っていたため
- **引き続き有効** — deny ログを元にルール追加を判断するという運用方針そのもの
