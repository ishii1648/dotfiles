...
- `ssh` を fish 関数でラップし、接続先ホスト名を引数から取り出して `⇢ <host>` にリネームする
...
- 追加コンポーネント無し。fish 関数 2 本 + conf.d 1 本で完結し、herdr 本体の更新に追従しやすい
...
| `tests/fish-functions.bats` | `_ssh_tab_host` の引数解析テストを追加。撤去済み `__tm_session_name` のテスト 3 件を削除 |
