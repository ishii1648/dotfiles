...
show_help() {
    ...
    echo ""
    echo "Check if dotfiles symlinks are properly configured."
    echo ""
    echo "Options:"
    echo "  -f, --fix    Fix missing symlinks automatically"
...
# dotfiles リポジトリのパスを取得（このスクリプトはリポジトリルートに配置）
...
check_symlink() {
    ...
    if [[ ! -L "$link_path" ]]; then
        ...
        if [[ "$FIX_MODE" == "true" ]]; then
            echo "    ⚠ Manual fix required (data loss risk): rm -rf $link_path && ln -s $expected_target $link_path"
        else
    ...
    if [[ "$actual_normalized" != "$expected_normalized" ]]; then
        ...
        if [[ "$FIX_MODE" == "true" ]]; then
            echo "    ⚠ Manual fix required: rm $link_path && ln -s $expected_target $link_path"
        else
