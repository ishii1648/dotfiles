...
ラベル付きの箇条書き（「設計上の判断」等）は **太字 + em ダッシュ**（` — `）で書く。コロンは使わない。
...
理由: textlint（`@textlint-ja/ai-writing/no-ai-list-formatting`）が「強調（`**`）とコロン（`:`）の組み合わせは機械的な印象」として指摘するため。em ダッシュは `docs/issues.md` のサマリ表でも使っている既存の書き方で、太字ラベルの一覧性を保ったまま指摘を回避できる。

過去 ADR 30 本・128 箇所は一括で移行済み。**新規 ADR で `- **ラベル**: 説明` 形式を使わないこと。**
