...
ADR には設計の合意が既に記録されているため、Plan Mode は使用しない。

- `## 決定` が空または `（未定）` かつ `## 設計案` に複数案が列挙されている場合のみ、AskUserQuestion でユーザーに案を選択させる
- それ以外は `## 決定`（または `## 設計案`）の内容をそのまま実装方針とする
- 実装タスクを **独立して並列実行できる単位** に分割し、依存関係を整理する
  - 例: 「設定ファイル A の追加」「設定ファイル B の追加」は並列 / 「シンボリックリンク作成」は A・B の完了後
- ファイル削除が含まれる場合は削除対象を設計案から特定して `git rm` を使用する
- 設計に「移動」操作が含まれる場合、**移動元の削除タスクと移動先の作成タスクが両方**含まれていることを確認する
